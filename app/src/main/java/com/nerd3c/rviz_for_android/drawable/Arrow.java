 ...
 */
package com.nerd3c.rviz_for_android.drawable;
...
import com.nerd3c.renderer.Vertices;
import com.nerd3c.renderer.shapes.BaseShape;
...
public class Arrow extends BaseShape {
	...
	public static Arrow newArrow(Camera cam, float cylRadius, float coneRadius, float cylHeight, float coneHeight) {
		...
		for(float theta = 0; theta <= (TWO_PI + dTheta); theta += dTheta) {
			...
			sideVertices[sideVidx++] = (float)Math.cos(theta)*CONE_RADIUS; // X
			sideVertices[sideVidx++] = (float)Math.sin(theta)*CONE_RADIUS; // Y
			sideVertices[sideVidx++] = CONE_Z_OFFSET; // Z

			sideNormals[sideNidx++] = (float)Math.cos(theta) / 1.224f; // X
			sideNormals[sideNidx++] = (float)Math.sin(theta) / 1.224f; // Y
			...
			// X
			bottomVertices[capVidx++] = (float)Math.cos(TWO_PI - theta)*CONE_RADIUS;
			// Y
			bottomVertices[capVidx++] = (float)Math.sin(TWO_PI - theta)*CONE_RADIUS;
			// Z
		...
		for(float theta = 0; theta <= (TWO_PI + dTheta); theta += dTheta) {
			sideVertices[sideVidx++] = (float)Math.cos(theta)*CYL_RADIUS; // X
			sideVertices[sideVidx++] = (float)Math.sin(theta)*CYL_RADIUS; // Y
			...
			// X
			bottomVertices[capVidx++] = (float)Math.cos(TWO_PI - theta)*CYL_RADIUS;
			// Y
			bottomVertices[capVidx++] = (float)Math.sin(TWO_PI - theta)*CYL_RADIUS;
			// Z
