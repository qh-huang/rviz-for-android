...
import com.nerd3c.renderer.shapes.TriangleStripShape;
...
public class Ring extends TriangleStripShape {
	...
	public static Ring newRing(Camera cam, float Rinner, float Router, int segments) {
		...
		for(float theta = (float) (2f*Math.PI); theta >= 0; theta -= dTheta) {
			// Outer
			vertices[idx++] = 0f;
			vertices[idx++] = (float)Math.sin(theta) * Router;
			vertices[idx++] = (float)Math.cos(theta) * Router;
			
			
			// Inner
			vertices[idx++] = 0f;
			vertices[idx++] = (float)Math.sin(theta) * Rinner;
			vertices[idx++] = (float)Math.cos(theta) * Rinner;
